  ...
  it.live(
    ...
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        ...
        expect(Object.keys(result.tools).toSorted()).toEqual(Object.keys(tools).toSorted())
        expect(result.preview.tools.every((tool) => tool.included)).toBe(true)
      }),
    ),
  )

  it.live(
    "keeps a historical tool optional when compact schemas fit",
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const compact = yield* SessionCompaction.Service
        const tools = Object.fromEntries(
        ...
        expect(result.tools.read).toBeDefined()
        expect(result.tools.grep).toBeDefined()
        expect(result.tools.historical).toBeDefined()
        expect(result.preview.tools.find((tool) => tool.name === "historical")).toMatchObject({
          required: false,
          included: true,
        })
      }),
